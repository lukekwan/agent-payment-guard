# Agent Payment Control: examples
...
```bash
curl --fail-with-body -X GET 'https://api.example.invalid/v1/x402/agent/payment-risk-gateway'
```

### JavaScript / TypeScript

```js
const response = await fetch("https://api.example.invalid/v1/x402/agent/payment-risk-gateway", { method: "GET" });
...
request = Request("https://api.example.invalid/v1/x402/agent/payment-risk-gateway", method="GET")
