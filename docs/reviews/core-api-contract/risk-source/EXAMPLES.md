...
```bash
curl --fail-with-body -X POST 'https://api.example.invalid/v1/risk-source/address-risk'
```

### JavaScript / TypeScript

```js
const response = await fetch("https://api.example.invalid/v1/risk-source/address-risk", { method: "POST" });
...
request = Request("https://api.example.invalid/v1/risk-source/address-risk", method="POST")
