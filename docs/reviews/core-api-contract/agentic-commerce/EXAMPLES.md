# Agentic Commerce Preflight: examples
...
```js
const response = await fetch("https://api.example.invalid/v1/agentic-commerce/preflight", { method: "POST" });
...
request = Request("https://api.example.invalid/v1/agentic-commerce/preflight", method="POST")
