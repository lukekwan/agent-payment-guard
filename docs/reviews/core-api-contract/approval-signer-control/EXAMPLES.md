# Approval & Signer Control: examples
...
```bash
curl --fail-with-body -X POST 'https://api.example.invalid/v1/approval-signer/evaluate'
```

### JavaScript / TypeScript

```js
const response = await fetch("https://api.example.invalid/v1/approval-signer/evaluate", { method: "POST" });
...
request = Request("https://api.example.invalid/v1/approval-signer/evaluate", method="POST")
