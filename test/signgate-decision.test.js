...
test("DEV-SG-001 auth and tenant binding fail closed", async () => {
  ...
  agentCredential.rotation_started_at = "2026-07-19T00:00:00.000Z";
  ...
  predecessor.created_at = "2026-07-19T00:01:00.000Z";
  assert.equal((await postDecision(baseRequest({ request_id: "req_impossible_rotation_chronology" }), env)).status, 401);
  predecessor.created_at = "2026-07-18T23:59:00.000Z";
  agentCredential.rotation_started_at = null;
  assert.equal((await postDecision(baseRequest({ request_id: "req_missing_rotation_started_at" }), env)).status, 401);
  agentCredential.rotation_started_at = "2026-07-19T00:00:00.000Z";
  ...
  agentCredential.rotation_started_at = null;
  agentCredential.rotation_expires_at = null;
  agentCredential.allowed_environments_json = null;
  const nullEnvironments = await postDecision(baseRequest({ request_id: "req_credential_null_environments" }), env);
  assert.equal(nullEnvironments.status, 403);
  assert.deepEqual((await nullEnvironments.json()).reason_codes, ["ALLOWED_ENVIRONMENTS_JSON_MALFORMED"]);
  agentCredential.allowed_environments_json = "";
  const emptyStringEnvironments = await postDecision(baseRequest({ request_id: "req_credential_empty_string_environments" }), env);
  assert.equal(emptyStringEnvironments.status, 403);
  assert.deepEqual((await emptyStringEnvironments.json()).reason_codes, ["ALLOWED_ENVIRONMENTS_JSON_MALFORMED"]);
  ...
  agentCredential.allowed_action_types_json = JSON.stringify(["deploy_change"]);
  agentCredential.allowed_action_types_json = null;
  const nullActionTypes = await postDecision(baseRequest({ request_id: "req_credential_null_action_types" }), env);
  assert.equal(nullActionTypes.status, 403);
  assert.deepEqual((await nullActionTypes.json()).reason_codes, ["ALLOWED_ACTION_TYPES_JSON_MALFORMED"]);
  agentCredential.allowed_action_types_json = JSON.stringify(["deploy_change"]);
  ...
  agentCredential.allowed_services_json = null;
  const nullServices = await postDecision(baseRequest({ request_id: "req_credential_null_services" }), env);
  assert.equal(nullServices.status, 403);
  assert.deepEqual((await nullServices.json()).reason_codes, ["ALLOWED_SERVICES_JSON_MALFORMED"]);
  agentCredential.allowed_services_json = JSON.stringify(["signgate-worker"]);
  agentCredential.scopes_json = null;
  const nullScopes = await postDecision(baseRequest({ request_id: "req_credential_null_scopes" }), env);
  assert.equal(nullScopes.status, 403);
  assert.deepEqual((await nullScopes.json()).reason_codes, ["SCOPES_JSON_MALFORMED"]);
  agentCredential.scopes_json = JSON.stringify(["decision:create:deploy_change"]);
